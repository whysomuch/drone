...
#import
import sensor, image, time, math#定义类
from pyb import LED
...
def check_dot(img):
    ...
    rects = None
    rects = img.find_blobs([(50, 180)], x_stride=rect_w, y_stride=rect_h ,merge=True)
    for rrect in rects:
        ...
        if use_bright_threshold:
            blobs = img.find_blobs([[Thresholds.value(),255]], invert=False, roi=rrect.rect(), merge=True)
