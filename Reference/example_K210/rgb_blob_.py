...
while (act == 1):
    clock.tick()
    img = sensor.snapshot()#.histeq(adaptive=True, clip_limit=1.2)#.lens_corr(1.1).histeq(adaptive=True, clip_limit=1.2)
    #boost_ratio = get_morph_boost_ratio(img, red_threshold, rect_w, rect_h)
    #img.morph(1,  sharpen_kernel, boost_ratio)
    blobs = img.find_blobs([green_threshold], invert=False, merge=True)
    for b in blobs:
        blob_blob = img.find_blobs([green_threshold], roi=b[0:4], invert=False, merge=True)
        for bb in blob_blob:
