...
def point_detection():
    corner_sample = is_patch_white(img, b.x() + b.w() // 4, b.y() + b.h() // 4)
    quater_sample = is_patch_white(img, b.x() + 4 * b.w() // 5, b.y() + 3 * b.h() // 4)
    center_sample = is_patch_white(img, b.x() + b.w() // 2, b.y() + b.h() // 2)
    right_sample = is_patch_white(img, b.x() + b.w(), b.y() + b.h() // 2)
    base_sample = is_patch_white(img, b.x() + 4 * b.w() // 5, b.y() + b.h())
    stroke_sample = is_line_white(img, b.x() + 4 * b.w() // 5, b.y() + b.h(), b.x() + 4 * b.w() // 5, b.y() + 3 * b.h() // 4)
    tilt_sample = is_line_white(img, b.x() + 3 * b.w() // 4, b.y(), b.x() + b.w(), b.y() + b.h() // 2)
...
while (act == 1):
    ...
    result_list_init()
    boost_ratio = get_morph_boost_ratio(img, blob_threshold_1, rect_w, rect_h)
    img.morph(1,  sharpen_kernel, boost_ratio)
    blobs = img.find_blobs([blob_threshold_2], invert=False, merge=True)
    blobs = limit_condition(blobs)
    for b in blobs:
        ratio = b.w() / b.h()
        if ratio > given_length_width_rate:
            locked = True
        else:
            locked = False
        x_offset = b.x() + b.w() // 2
