...
共有 4 个 SPI 设备， 其中 SPI0 、SPI1、 SPI3 只能工作在主机模式下，
SPI2 只能工作在从机模式时下， 在 MaixPy 上，
SPI3 已经用来连接了 SPI Flash 作为保留硬件资源。
支持 1/2/4/8 线全双工模式，
在MaixPy中，目前只支持标准（摩托罗拉）4线全双工模式（即SCK，MOSI，MISO，CS 四个引脚）
最高传输速率 45M：1/2主频，约 200Mbps
支持 DMA
4个可配置任意引脚的硬件片选

一、构造函数
spi = machine.SPI(id, mode=SPI.MODE_MASTER, baudrate=500000, polarity=0, phase=0, bits=8,
firstbit=SPI.MSB, sck, mosi, miso, cs0, cs1, cs2, cs3)
通过指定的参数新建一个 SPI 对象

参数
id： SPI ID，取值范围[0,4]，目前只支持0和1、4，并且只能是主机模式，2只能作为从机，目前未实现，
                                        3保留,4使用软模拟SPI（.SPI_SOFT）
mode： SPI 模式， MODE_MASTER 或者MODE_MASTER_2或者MODE_MASTER_4
                                        或者MODE_MASTER_8或者MODE_SLAVE，
                                        目前只支持MODE_MASTER
baudrate： SPI 波特率（频率）
...
类似构造函数
spi.init(id, mode=SPI.MODE_MASTER, baudrate=500000, polarity=0, phase=0,
bits=8, firstbit=SPI.MSB, sck, mosi, miso, cs0)
参数
与构造函数相同
返回值
无
...
spi = SPI(SPI.SPI1, mode=SPI.MODE_MASTER, baudrate=10000000, polarity=0, phase=0, bits=8,
